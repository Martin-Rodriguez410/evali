...
from .pdf_export import exportar_datos_pdf
from .import_data import importar_datos_excel
from .utils import normalize_rut
from django.views.decorators.http import require_POST
from django.forms.models import model_to_dict

@login_required
def registro_parto(request, madre_id=None):
    # Si se pasa un madre_id, obtener la madre y prellenar el formulario
    madre_prellenada = None
    if madre_id:
        try:
            madre_prellenada = get_object_or_404(Madre, id=madre_id)
        except:
            pass
    ...
    if request.method == 'POST':
        ...
        # Prellenar si hay madre_prellenada
        if madre_prellenada:
            initial_madre = {
                'rut': madre_prellenada.rut,
                'nombres': madre_prellenada.nombres,
                'apellidos': madre_prellenada.apellidos,
                'fecha_nacimiento': madre_prellenada.fecha_nacimiento,
                'estado_civil': madre_prellenada.estado_civil,
                'prevision': madre_prellenada.prevision,
                'direccion': madre_prellenada.direccion,
                'telefono': madre_prellenada.telefono,
            }
            form = PartoCompletoForm(
                prefixes=('madre','parto','recien'),
                initial={'madre': initial_madre}
            )
        else:
            form = PartoCompletoForm(prefixes=('madre','parto','recien'))
    
    # Crear formulario de madre para modal
    madre_create_form = MadreForm(prefix='madre_create')
    
    return render(request, 'registros/registro_parto.html', {
        'form': form,
        'madre_create_form': madre_create_form,
        'titulo': 'Nuevo Registro de Parto',
        'madre_prellenada': madre_prellenada
    })
...
def lista_madres(request):
    query = request.GET.get('q', '')
    madres = Madre.objects.select_related('created_by').order_by('-created_at')
    ...
    paginator = Paginator(madres, 15)
    page = request.GET.get('page')
    madres_paginadas = paginator.get_page(page)
    
    # Manejar creación de nueva madre
    if request.method == 'POST':
        form = MadreForm(request.POST)
        if form.is_valid():
            try:
                madre = form.save(commit=False)
                madre.created_by = request.user
                madre.save()
                ...
                messages.success(request, f'Madre creada exitosamente - RUT: {madre.rut}, Nombre: {madre.nombres} {madre.apellidos}')
                return redirect('registros:lista_madres')
            except Exception as e:
                logger = logging.getLogger(__name__)
                logger.error('Error al crear madre: %s', str(e))
                messages.error(request, f'Error al crear la madre: {str(e)}')
        else:
            # Si hay errores, los pasamos al contexto
            return render(request, 'registros/lista_madres.html', {
                'madres': madres_paginadas,
                'query': query,
                'titulo': 'Lista de Madres',
                'form': form
            })
    else:
        form = MadreForm()
...
@login_required
def exportar_partos_pdf(request):
    """Exportar partos a PDF dentro de un rango de fechas."""
    start = request.GET.get('start')
    end = request.GET.get('end')
    ...
    return exportar_datos_pdf(fecha_inicio, fecha_fin)

@login_required
def importar_partos(request):
    if request.method == 'POST':
        ...
        if result['success']:
            ...
            msg = f"Importación exitosa. Creados: {counts.get('madres', 0)} Madres, {counts.get('partos', 0)} Partos, {counts.get('rn', 0)} RNs."
            messages.success(request, msg)
            
            if result.get('errors'):
                messages.warning(request, f"Se encontraron algunos errores no críticos: {len(result['errors'])}")
                # Podríamos mostrar los errores en detalle si fuera necesario
        else:
            messages.error(request, f"Error en la importación: {result.get('message')}")
            
        return redirect('registros:lista_partos')

    return render(request, 'registros/importar_partos.html', {'titulo': 'Importar Registros'})
